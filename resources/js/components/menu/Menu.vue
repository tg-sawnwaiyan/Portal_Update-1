<template>
    <div>
        ...
        <nav class="navbar navbar-expand-lg  main-header" v-if="!$auth.check() || this.visit">
            <div class="container nav-warp d-flex">
            <div class="d-flex align-items-center">
                <a class="navbar-brand logo-text" href="/">
                LOGO <span>HERE</span> 
                </a>
                <span v-if="$auth.check() && this.visit" @click="gotoDash()">Dashboard</span>
  ...
  export default {
    ...
    mounted() {
        ...
        if(localStorage.getItem("visit")){
            ...
            console.log(this.visit + "menu1")
            console.log("auth "+this.$auth.check())
        }
        else{
            if(this.$auth.check()){
                localStorage.setItem('visit', false);
                console.log(this.visit + "menu2")
            }
            else{
                localStorage.setItem('visit', true);
                console.log(this.visit + "menu3")
            }
        }
