<template>
    <div>
        <div id="main" class="main-content pad-free" v-if="!$auth.check() && this.$route.path !== '/register' && this.$route.path !== '/login'">
        ...
        <div id="main" class="full-content pad-free" v-if="$auth.check()">
            <button onclick="topFunction()" id="myBtn">Top</button>
            <HeaderMenu></HeaderMenu>
        ...
        <div v-if="this.$route.path === '/register' || this.$route.path === '/login'">
            <router-view :key="$route.fullPath"></router-view>
        </div>
    </div>
</template>
